...
| Gemini API calls (ticket/question content) | Global endpoint, no residency guarantee | Google's data-residency page: requests to a global endpoint "may be processed in any Google Cloud location around the world, and therefore don't provide any data residency guarantees". The Gemini API this book calls has no region setting I could find. On Google's enterprise platform `gemini-3.6-flash` can be pinned to a US or EU multi-region, not a single country; this book never used that platform |

**A real, previously-unexamined finding**: this book's own single live product already spans three jurisdictions for a single request. A South African user's own identity token is issued and verified against an EU-based Auth0 tenant; their reorder question and the agent's own answer are sent to an endpoint with no residency guarantee at all; the resulting workflow state lives in a South African Postgres instance. None of this was a deliberate architecture decision, it's the sum of independent, correct choices (Auth0's free-tier tenant defaulted to EU when created in chapter 6; Fly's `jnb` region was chosen in chapter 10 for reasons unrelated to residency; `gemini-3.6-flash` was this series' own model choice since Book 2). A real compliance review would need to name this explicitly, not assume a single-region deployment implies single-jurisdiction data handling.
